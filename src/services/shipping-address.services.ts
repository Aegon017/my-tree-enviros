...
export const shippingAddressService = {
    async index() {
        return await api.get<{ addresses: ShippingAddress[] }>("/shipping-addresses");
    },

    async store(payload: CreateShippingAddressPayload) {
        return await api.post<{ address: ShippingAddress }>("/shipping-addresses", payload);
    },

    async show(id: number) {
        return await api.get<{ address: ShippingAddress }>(`/shipping-addresses/${id}`);
    },

    async update(id: number, payload: UpdateShippingAddressPayload) {
        return await api.put<{ address: ShippingAddress }>(`/shipping-addresses/${id}`, payload);
    },
    ...
    async setDefault(id: number) {
        return await api.post<{ address: ShippingAddress }>(`/shipping-addresses/${id}/set-default`);
    },
};
