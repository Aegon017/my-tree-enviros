        ...
        (set, get) => ({
            ...
            addToCart: async (payload) => {
                ...
                if (payload.type === "product") {
                    ...
                    if (typeof firstImage === 'string') {
                        ...
                        imageUrl = firstImage.url || firstImage.original_url || firstImage.src || firstImage.image_url;
                    }
                    ...
                    newItem = {
                        ...
                        price: payload.variant?.selling_price || payload.variant?.original_price || 0,
                        image_url: imageUrl,
                        variant: {
                    ...
                    const planData = tree.plan_prices?.find((pp: any) => pp.id === payload.plan_price_id);

                    const availablePlans = tree.plan_prices?.reduce((acc: any[], pp: any) => {
            ...
            updateItem: async (id, payload) => {
                ...
                if (item) {
                    ...
                    if (payload.plan_price_id && item.type !== "product" && item.available_plans) {
                        const selectedPlan = item.available_plans.find((plan: any) =>
                            plan.plan_prices.some((pp: any) => pp.id === payload.plan_price_id)
                        );
                        if (selectedPlan) {
