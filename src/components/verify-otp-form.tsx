...
const verifyOtp = async ( _url: string, { arg }: { arg: { mobile: string; otp: string } } ) => {
  const response = await api.post( "/api/verify-otp", arg );
  return response.data;
};

const resendOtp = async ( _url: string, { arg }: { arg: { mobile: string } } ) => {
...
export function VerifyOtpForm( { className, ...props }: React.ComponentProps<"div"> ) {
  ...
  const { trigger: verifyTrigger, isMutating: isVerifying } = useSWRMutation( "/api/verify-otp", verifyOtp );
  const { trigger: resendTrigger, isMutating: isResending } = useSWRMutation( "/api/resend-otp", resendOtp );
