...
import { CheckCircle2, Edit, Loader2, MapPin, Plus, Trash2 } from "lucide-react";
...
import { ShippingAddress } from "@/services/shipping-address.services";
...
const AddressCard = memo(({ address, isSelected, onSelect, onEdit, onDelete }: AddressCardProps) => (
  <Card className={`cursor-pointer transition-all gap-0 ${isSelected ? "border-primary border-2" : "hover:border-primary/50"}`} onClick={() => onSelect(address.id)}>
    ...
    <CardContent className="px-4 pb-4 sm:px-6">
      ...
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" size="sm" className="flex-1 sm:flex-none text-xs sm:text-sm" onClick={(e) => { e.stopPropagation(); onEdit(address); }}>
          <Edit className="h-3 w-3 sm:h-4 sm:w-4 mr-1" /> Edit
        </Button>
        <Button variant="outline" size="sm" className="flex-1 sm:flex-none text-xs sm:text-sm" onClick={(e) => { e.stopPropagation(); onDelete(address.id); }}>
...
const AddressForm = ({ open, onOpenChange, editingAddress, onSubmit, isSubmitting }: {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  editingAddress: ShippingAddress | null;
  onSubmit: (values: FormValues, setFieldError: (name: keyof FormValues | string, error: { type?: string; message?: string }) => void) => Promise<void>;
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    const setFieldError = (name: keyof FormValues | string, error: { type?: string; message?: string }) => {
      form.setError(name as any, { type: error.type ?? "server", message: error.message ?? "" });
    };
    ...
    try {
      ...
      // if parent didn't map errors, show a generic message
      if (!err?.errors) {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            <div className="space-y-2">
              <LocationPicker latitude={form.watch("latitude")} longitude={form.watch("longitude")} onLocationChange={handleLocationChange} />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField control={form.control} name="postal_code" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Postal Code" {...field} onChange={(e) => { field.onChange(e); handlePostalCodeChange(e.target.value); }} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            ...
            {postOffices.length > 0 && (
              <FormField control={form.control} name="post_office_name" render={({ field }) => (
                <FormItem>
...
export default function ShippingAddresses({ onSelect, selectedAddressId: externalSelectedId }: ShippingAddressesProps) {
  ...
  const onSubmit = async (values: FormValues, setFieldError: (name: keyof FormValues | string, error: { type?: string; message?: string }) => void) => {
    ...
    try {
      ...
      // map backend validation errors into form
      const validationErrors = err?.errors || err?.data?.errors || err?.data?.validation_errors;
      ...
      // rethrow so AddressForm can display a generic error
      throw err;
    } finally {
  ...
  return (
    <div className="container mx-auto">
      ...
      {addresses.length > 0 ? (
        <div className="grid grid-cols-1 gap-4">
          {addresses.map((address) => (
            <AddressCard key={address.id} address={address} isSelected={selectedAddressId === address.id} onSelect={handleSelectAddress} onEdit={handleEdit} onDelete={setDeleteId} />
          ))}
        </div>
      ) : (
        <Card>
      ...
      <AddressForm open={isDialogOpen} onOpenChange={handleDialogOpenChange} editingAddress={editingAddress} onSubmit={onSubmit} isSubmitting={isSubmitting} />
