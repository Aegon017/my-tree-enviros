...
import { reverseGeocode, getPostOffices, saveShippingAddress } from "@/lib/apiAddress";
...
export default function AddressForm( { onSaved }: { onSaved?: () => void } ) {
        ...
        ( async () => {
            try {
                ...
                if ( data.postal_code ) {
                    ...
                    if ( !list || list.length === 0 ) toast.error( "No Post Offices found for this pincode." );
                }
            } catch {
    ...
    async function onSubmit( values: AddressInputs ) {
        ...
        const selectedPO = postOffices.find( ( p ) => p.name === values.post_office_name );
        const geo = {
            lat: position[ 0 ],
            lng: position[ 1 ],
            area: values.area || "",
            ...
            post_office_branch_type: values.post_office_branch_type || selectedPO?.branch_type || "",
        };
        try {
            if ( isAuthenticated ) {
                ...
                <form onSubmit={ form.handleSubmit( onSubmit ) } className="space-y-3">
                    ...
                    <FormField control={ form.control } name="address" render={ ( { field } ) => (
                        <FormItem>
                            ...
                            <FormControl><Input placeholder="Flat No, Street" { ...field } /></FormControl>
                            <FormMessage />
                        </FormItem>
                    ) } />
                    ...
                    <FormField control={ form.control } name="post_office_name" render={ ( { field } ) => (
                        <FormItem>
                            ...
                            <Select onValueChange={ field.onChange } value={ field.value }>
                                ...
                                <SelectContent>
                                    { loadingPosts && <SelectItem value="loading">Loading...</SelectItem> }
                                    { !loadingPosts && postOffices.filter( po => po.name && po.name.trim() !== "" ).map( po => (
                                        <SelectItem key={ po.name } value={ po.name }>{ po.name } — { po.branch_type }</SelectItem>
                                    ) ) }
                                </SelectContent>
                            </Select>
                            <FormMessage />
