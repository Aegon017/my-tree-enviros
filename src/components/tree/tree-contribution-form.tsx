...
export default function TreeContributionForm({ tree, pageType }: { tree: Tree; pageType: "sponsor" | "adopt" }) {
    ...
    const planOptions = useMemo(() => {
        ...
        return tree.plan_prices.map(pp => ({
            ...
            durationDisplay: pp.plan.duration > 1 ? `${pp.plan.duration} Years` : `${pp.plan.duration} Year`,
    ...
    const adoptableLimit = pageType === "adopt" ? tree?.adoptable_count ?? 0 : Infinity;
        ...
        <>
            <Card className="border-l-4 border-l-primary">
                <CardContent className="p-6">
                    <h3 className="text-xl font-semibold mb-6">Configure Your {pageType === "sponsor" ? "Sponsorship" : "Adoption"}</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center border rounded-md bg-background justify-between">
                                ...
                                <Input type="number" min={1} max={999} value={quantity} onChange={e => setQuantity(Math.max(1, Number(e.target.value || 1)))} className="w-16 text-center border-0" />
                                <Button variant="ghost" size="icon" onClick={() => setQuantity(q => pageType === "adopt" ? Math.min(adoptableLimit, q + 1) : q + 1)}>+</Button>
                            </div>
                            {pageType === "adopt" && (adoptableLimit > 0 ? <p className="text-sm text-green-600">{adoptableLimit} available</p> : <p className="text-sm text-red-600">No trees available</p>)}
                        </div>

                        <div className="space-y-3">
                            <Label>Duration</Label>
                            <Select value={String(selectedYears)} onValueChange={v => setSelectedYears(Number(v))}>
                                <SelectTrigger className="h-11 w-full"><SelectValue placeholder="Select duration" /></SelectTrigger>
                                <SelectContent>
                                    {planOptions.map(p => <SelectItem key={p.planPriceId} value={String(p.duration)}>{p.durationDisplay}</SelectItem>)}
                                </SelectContent>
                            </Select>
                        </div>
                    </div>
                        ...
                        <div className="bg-primary/5 p-4 rounded-md border mt-6">
                            <div className="flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">{quantity} × {selectedPlan.duration} year(s)</p>
                                </div>
            ...
            <form className="space-y-6" onSubmit={handleSponsorNow}>
                <Card className="border-l-4 border-l-primary">
                    <CardContent className="p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <Label>Name</Label>
                                <Input placeholder="Name on certificate" {...form.register("name")} />
                                <p className="text-sm text-destructive mt-1">{form.formState.errors.name?.message as string || ""}</p>
                            </div>

                            <div>
                                <Label>Occasion</Label>
                                <Input placeholder="Birthday, Anniversary, etc." {...form.register("occasion")} />
                                <p className="text-sm text-destructive mt-1">{form.formState.errors.occasion?.message as string || ""}</p>
                            </div>

                            <div className="md:col-span-2">
                                <Label>Special Message</Label>
                                <Textarea rows={3} placeholder="A message for the certificate" {...form.register("message")} />
                                <p className="text-sm text-destructive mt-1">{form.formState.errors.message?.message as string || ""}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <div className="flex gap-3">
                    <AddToCartButton
                        ...
                        quantity={quantity}
                        dedication={{ name: watchName, occasion: watchOccasion, message: watchMessage }}
                    ...
                    <Button className="flex-1" disabled={!selectedPlan} type="submit"> {pageType === "sponsor" ? "Sponsor" : "Adopt"} Now </Button>
                </div>
            </form>
