...
export function EcommerceCard({ product }: Props) {
  ...
  const { trigger: addWishlistTrigger } = useSWRMutation(
    "/wishlist/add",
    addToWishlist,
  );
  const { trigger: removeWishlistTrigger } = useSWRMutation(
    "/wishlist/remove",
    removeFromWishlist,
  );
  const { trigger: addCartTrigger, isMutating: isAddingToCart } =
    useSWRMutation("/cart/add", addToCart);
