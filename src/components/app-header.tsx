...
const LOCATIONS = [ "Hyderabad", "Bangalore", "Mumbai", "Delhi", "Chennai", "Kolkata" ];
const TOP_BAR_LINKS = [
...
export default function Header() {
  ...
  const cartItems = useSelector( ( state: RootState ) => state.cart.items?.data ?? [] );

  return (
    <>
      <TopNotice />
      <div className={ cn( "bg-background z-50", isMobile && "sticky top-0" ) }>
        <TopBar user={ isAuthenticated } logout={ logout } />
        <HeaderMiddle user={ isAuthenticated } />
      </div>
  ...
  return (
    <div className="bg-primary text-primary-foreground px-2 sm:px-4">
      <div className="container mx-auto max-w-6xl py-1.5 sm:py-2 flex justify-center items-center gap-2 sm:gap-4">
        <span className="font-semibold text-xs sm:text-sm text-center">{ NOTICE_TEXT }</span>
        <Button
          variant="ghost"
          size="sm"
    ...
    <div className="bg-muted text-muted-foreground text-xs">
      <div className="container mx-auto max-w-6xl py-1.5 sm:py-2 flex justify-between items-center px-2">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              ...
              <span className="truncate max-w-[80px] xs:max-w-[100px] sm:max-w-none">{ selectedLocation }</span>
          ...
          <DropdownMenuContent align="start" className="w-40 max-h-60 overflow-y-auto">
            { LOCATIONS.map( location => (
              <DropdownMenuItem
                ...
                className={ cn( "cursor-pointer text-xs", selectedLocation === location && "bg-accent" ) }
              >
                { location }
              </DropdownMenuItem>
            ) ) }
          ...
          { TOP_BAR_LINKS.map( ( item, index ) => {
            ...
            return (
              <div key={ item.name } className="flex items-center">
                ...
                { index < TOP_BAR_LINKS.length - 1 && <Separator orientation="vertical" className="h-3 bg-border mx-1" /> }
              </div>
            );
          } ) }
          { user && (
            <Button variant="ghost" onClick={ logout } className="text-xs text-muted-foreground hover:text-primary transition-colors px-2 h-6">
              Sign Out
            </Button>
          ) }
        </div>
        ...
        <div className="flex md:hidden items-center gap-2">
          { user ? (
            <Button variant="ghost" onClick={ logout } className="text-xs text-muted-foreground hover:text-primary transition-colors">
              Sign Out
            </Button>
          ) : (
            <>
              ...
              <Link href="/sign-up" className="text-xs bg-primary text-white px-3 py-1 rounded hover:bg-primary/90 transition-colors">
                Sign Up
              </Link>
            </>
          ) }
      ...
      <div className="container mx-auto max-w-6xl px-3 sm:px-4 flex items-center justify-between">
          ...
          { isMobile ? <MobileNavigation user={ user } /> : (
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <Link href={ `tel:${ PHONE_NUMBER }` } className="font-semibold text-sm hover:text-primary transition-colors">
                  { PHONE_NUMBER }
                </Link>
              </div>
            </div>
        ...
        <div className="flex items-center space-x-1 sm:space-x-2">
          { ICON_LINKS.map( ( { href, icon: Icon, label } ) => {
            if ( label === "Cart" ) {
              return (
                <Button key={ label } variant="ghost" size="icon" asChild className="relative text-muted-foreground hover:text-primary h-8 w-8 sm:h-9 sm:w-9" aria-label={ label }>
                  <Link href="/cart">
            ...
            return (
              <Button key={ label } variant="ghost" size="icon" asChild className="text-muted-foreground hover:text-primary h-8 w-8 sm:h-9 sm:w-9" aria-label={ label }>
                <Link href={ href }>
        ...
        <div className="flex justify-center min-w-max">
          { MAIN_NAV_ITEMS.map( item => (
            <Link key={ item.href } href={ item.href } className={ cn(
              ...
              { item.label }
              { pathname === item.href && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary" /> }
            </Link>
          ) ) }
        </div>
      </div>
      ...
      <SheetContent side="left" className="w-[280px] sm:w-[340px] p-0">
          ...
          { MAIN_NAV_ITEMS.map( item => (
            <SheetClose asChild key={ item.href }>
              <Link href={ item.href } className={ cn(
                ...
                pathname === item.href ? "text-primary bg-primary/5" : "text-foreground hover:bg-accent"
              ) }>
                { item.label }
              </Link>
            </SheetClose>
        ...
        <div className="mt-6 pt-4 border-t mx-4">
          <div className="flex items-center gap-3 py-2">
            ...
            <Link href={ `tel:${ PHONE_NUMBER }` } className="font-medium text-sm hover:text-primary transition-colors">
              { PHONE_NUMBER }
            </Link>
          </div>
          ...
          <div className="mt-4 grid grid-cols-1 gap-1">
              ...
              return (
                <SheetClose asChild key={ item.name }>
                  <Link href={ item.href } className={ cn(
                    ...
                    item.highlight ? "text-white bg-primary rounded-md hover:bg-primary/90" : "text-muted-foreground hover:text-primary"
                  ) }>
                    { item.name }
                  </Link>
                </SheetClose>
            ...
            { user && (
              <Button variant="ghost" onClick={ logout } className="justify-start text-sm text-muted-foreground hover:text-primary px-2 py-2 h-auto">
                Sign Out
              </Button>
            ) }
          </div>
