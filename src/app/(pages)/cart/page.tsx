...
export default function CartPage() {
    ...
    const updateCartItem = useCallback( async ( productId: number, params: CartItem ) => {
        setUpdatingItems( ( prev ) => new Set( prev ).add( productId ) );
        try {
            await fetch( `${ process.env.NEXT_PUBLIC_BACKEND_API_URL }/api/cart/add/${ productId }`, {
                method: 'POST',
                headers: {
    ...
    const removeItem = useCallback( async ( itemId: number ) => {
        ...
        try {
            await fetch( `${ process.env.NEXT_PUBLIC_BACKEND_API_URL }/api/cart/remove/${ itemId }`, {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    Authorization: `Bearer ${ storage.getToken() }`
    ...
    if ( error ) return <div className="container mx-auto p-6">Failed to load cart</div>;
                                            ...
                                            <div className="flex items-center space-x-6">
                                                ...
                                                <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                                                    <div className="space-y-1">
                                                        <h3 className="font-semibold text-lg">{ item.ecom_product.name }</h3>
                                                        ...
                                                        <div className="flex items-center border rounded-md">
                                                            <Button
                                                                ...
                                                                onClick={ () => updateCartItem(
                                                                    ...
                                                                    { ...cartParams, quantity: Math.max( 1, item.quantity - 1 ) }
                                                                ) }
                                                            ...
                                                            <Input
                                                                ...
                                                                onChange={ ( e ) => {
                                                                    ...
                                                                    updateCartItem( item.ecom_product.id, { ...cartParams, quantity: newQuantity } );
                                                                } }
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={ () => updateCartItem(
                                                                    item.ecom_product.id,
                                                                    { ...cartParams, quantity: item.quantity + 1 }
                                                                ) }
                                                                disabled={ isUpdating }
                                                            >
                                                    ...
                                                    <div className="text-right">
                                                        <p className="text-lg font-bold">
                                                            ${ ( item.ecom_product.price * item.quantity ).toFixed( 2 ) }
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
