...
const Page = () => {
  const { data: trees, error, isLoading } = useSWR( `${ process.env.NEXT_PUBLIC_BACKEND_API_URL }/api/adopt-trees`, fetcher );
      ...
      <Section className="bg-background py-12">
        ...
        { trees.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-600">No trees available for adoption at the moment.</p>
          </div>
        ) : (
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 justify-center gap-6">
            { trees.map( ( tree: Tree ) => (
